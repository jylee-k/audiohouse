...
def optimize_invoices(items):
    """
    items = [{'id': 1, 'name': 'Sofa', 'price': 2600, 'quantity': 2, 'eligible': True}, ...]
    ...
    Discounts:    $300 off per $2,500 block in each invoice total (~12%)
    Cashback:     Earned from Invoice 1 eligible spend: $154 per $200 block, floored to $20
    Cashback cap: 20% of Invoice 2 total AFTER its discount, floored to $20
    ...
    b1 = LpVariable("b1", cat="Integer", lowBound=0)   # $200 eligible blocks in Invoice 1

    # --- Invoice Totals ---
    t1 = lpSum([items[i]["price"] * x[i] for i in range(len(items))])
    t2 = lpSum([items[i]["price"] * (items[i]["quantity"] - x[i]) for i in range(len(items))])

    # --- Discount Constraints (floor: d <= t / 2500) ---
    prob += d1 <= t1 / 2500
    prob += d2 <= t2 / 2500

    # --- Cashback Earned (only Invoice 1 eligible items) ---
    t1_eligible = lpSum(
        [items[i]["price"] * x[i] for i in range(len(items)) if items[i]["eligible"]]
    )
    prob += b1 <= t1_eligible / 200          # $200 blocks
